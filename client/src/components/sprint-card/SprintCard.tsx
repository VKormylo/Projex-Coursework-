...
type Props = {
  sprint: SprintDto
  /** Whether any sprint in this project is already active */
...
export default function SprintCard({
  ...
  const menuItems = [
    ...
    ...(sprint.status === 'active' && onClose
      ? [{ label: 'Закрити спринт', onClick: () => onClose(sprint) }]
      : []),
    ...(onDelete
      ? [{
          label: 'Видалити',
          ...
          title: sprint.status === 'active'
            ? 'Закрийте спринт перед видаленням'
            : undefined,
        }]
      : []),
  ]
  ...
  return (
    <div className="rounded-[10px] border border-[#e2e8f0] bg-white px-6 py-5">
      ...
      <div className="mt-3 flex items-center gap-6 border-t border-[#f1f5f9] pt-3 text-sm text-[#45556c]">
        <span>Задач: <strong className="text-[#0f172b]">{taskCount}</strong></span>
        <span>Story Points: <strong className="text-[#0f172b]">{totalSP}</strong></span>
      </div>
    </div>
  )
}
