...
export default function ProjectCard({
  ...
  const menuItems = [
    ...
    },
    {
      ...
      icon: <ArchiveIcon />,
      onClick: () =>
        onArchive({ ...project, status: isArchived ? "on_hold" : "archived" }),
    },
    {
      label: "Видалити",
      icon: <TrashIcon />,
  ...
  return (
    <article className="flex flex-col gap-4 rounded-[10px] border border-[#e2e8f0] bg-white px-6 pt-6 pb-px">
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          <div className="flex size-10 shrink-0 items-center justify-center rounded-lg bg-[#dbeafe]">
            <span className="text-sm font-bold leading-5 text-[#1447e6]">
              {key}
            </span>
          </div>
          <div>
            <p className="text-[18px] font-semibold leading-[27px] text-[#0f172b]">
              {project.name}
            </p>
      ...
      <p className="line-clamp-2 min-h-[40px] text-sm leading-5 text-[#45556c]">
        {project.description ?? "—"}
      </p>
